...
import { LogIn, Mail, Lock, AlertCircle, ArrowRight, User, HeartPulse } from "lucide-react";
...
export default function Login() {
  ...
  const { login, mentorLogin } = useAuth();
  ...
  return (
    <div className="min-h-[calc(100vh-8rem)] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full pointer-events-none z-0">
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-primary-200 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob"></div>
        <div className="absolute top-1/4 left-1/3 w-96 h-96 bg-success-200 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob" style={{ animationDelay: '3s' }}></div>
      </div>
      ...
      >
        <div className="bg-white/70 backdrop-blur-2xl rounded-3xl p-8 border border-white/50 shadow-2xl">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 shadow-inner mx-auto ${role === 'mentor' ? 'bg-secondary-100 text-secondary-600' : 'bg-primary-100 text-primary-600'}`}>
            {role === 'mentor' ? <HeartPulse size={32} strokeWidth={2.5} /> : <LogIn size={32} strokeWidth={2.5} />}
          </div>
          
          <h2 className="text-3xl font-extrabold text-text-900 text-center mb-2 tracking-tight">Welcome Back</h2>
          <p className="text-center text-text-500 mb-6 font-medium">Log in to your continuous safe space.</p>
          ...
          {error && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-danger-50 text-danger-600 p-4 rounded-xl flex items-center gap-3 mb-6 border border-danger-100 font-medium text-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className={`w-full py-4 text-white rounded-xl font-bold transition-all duration-300 flex items-center justify-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed mt-4 shadow-lg ${role === 'mentor' ? 'bg-secondary-600 hover:bg-secondary-500 shadow-secondary-500/30' : 'bg-primary-600 hover:bg-primary-500 shadow-primary-500/30'}`}
            >
              {loading ? "Verifying..." : (
                <>
          ...
          <p className="text-center text-text-500 mt-8 font-medium">
            Don't have an account yet? <Link to="/register" className="text-primary-600 hover:text-primary-700 font-bold transition-colors">Sign Up</Link>
          </p>
        </div>
      </motion.div>
    </div>
